...
## GSD Commands cho Game Dev (MMORPG)

Dưới đây là các lệnh `opencode` phổ biến nhất nên sử dụng cho dự án MMORPG này:

| Command | Mô tả | Ứng dụng trong Game Dev |
|---|---|---|
| `gsd-map-codebase` | Quét cấu trúc dự án | Dùng khi Agent mới join cần hiểu luồng Client-Server. |
| `gsd-plan-phase` | Lên kế hoạch chi tiết cho Phase | Dùng khi chuẩn bị làm Phase 3 (Inventory). Tự sinh file PLAN.md. |
| `gsd-execute-phase` | Thực thi plan đã lập | Viết code thật sự cho Phase dựa trên PLAN.md. |
| `gsd-debug` | Phân tích lỗi sâu | Dùng khi WebSocket bị disconnect vô cớ hoặc quái (Mob) đứng im không di chuyển. |
| `gsd-sketch` | Prototype giao diện | Dùng để vẽ nháp UI Inventory/Quest Board bằng HTML/CSS tĩnh trước khi code vào Phaser. |
| `gsd-add-tests` | Thêm unit tests | Bổ sung test cho hệ thống tính Dame (Combat System) hoặc XP logic. |
| `gsd-verify-work` | UAT theo hội thoại | Dùng sau mỗi phase để kiểm thử xem 2 tab mở lên có chạy đúng multiplayer không. |
| `gsd-pause-work` | Dừng và tạo handoff | Dùng khi hết giờ làm việc, tạo file context để hôm sau `gsd-resume-work` tiếp tục. |
